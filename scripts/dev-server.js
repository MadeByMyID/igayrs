...
const FALLBACK_404 = path.join(ROOT, '404.html');
...
function serveNotFound(response, headOnly = false) {
  let stat;
  try {
    stat = fs.statSync(FALLBACK_404);
  } catch {
  ...
  if (headOnly) {
    sendFileHead(response, FALLBACK_404, 404, stat);
    return;
  }

  serveFile(response, FALLBACK_404, 404, stat);
}
...
function createServer() {
  return http.createServer((request, response) => {
    ...
    const result = resolveRequestPath(url.pathname);
    if (result.error) {
    ...
    try {
      ...
      statusCode = 404;
      serveNotFound(response, request.method === 'HEAD');
      return;
    }

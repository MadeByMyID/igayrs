...
function testCiWorkflowRunsProjectChecksWithReadOnlyPermissions() {
  ...
  assert(workflow.includes('uses: actions/checkout@v4'), `${workflowPath}: expected pinned checkout major version`);
  assert(workflow.includes('uses: actions/setup-node@v4'), `${workflowPath}: expected Node setup step`);
...
function testDatasetUpdateWorkflowHasSafeBoundsAndFallbacks() {
  ...
  assert(workflow.includes('OUTPUT_DIR="assets/data/json"'), `${workflowPath}: dataset output should match the categorized assets path`);
  assert(workflow.includes('git add assets/data/json/igrs.meta.json assets/data/json/igrs.games.json assets/data/json/igrs.extra.json'), `${workflowPath}: commit step should stage categorized dataset files`);
